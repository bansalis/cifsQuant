...
def validate_project(config: dict, stages: list | None = None) -> list[dict]:
    ...
    if 'spatial' in stages:
        ...
        # Same template placeholders the production validator allows
        placeholder_markers = ('STRUCTURE_MARKER', 'IMMUNE_MARKER', 'T_CELL_MARKER',
                               'CD8_MARKER', 'CD4_MARKER')
        bad_pheno = []
        for name, defn in phenotypes.items():
            for key in ('positive', 'negative', 'anypos'):
                for m in defn.get(key, []):
                    if m not in display_names and m not in placeholder_markers:
...
def detect_channels_from_results(project_dir: Path) -> list[str]:
    ...
    for csv_path in results_dir.rglob('combined_quantification.csv'):
        try:
            ...
            # mcquant morphology/id columns are not imaging channels
            non_channel = {'x_centroid', 'y_centroid', 'cellid', 'cell_id', 'area',
                           'tile_x', 'tile_y', 'majoraxislength', 'minoraxislength',
                           'eccentricity', 'solidity', 'extent', 'orientation'}
            cols = [c for c in df.columns if c.lower() not in non_channel]
            return cols
        except Exception:
            continue
    return []
