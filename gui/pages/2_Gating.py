...
# app.py initializes this, but the page must also survive being opened directly
...
for marker, tab in zip(gateable, tabs):
    with tab:
        ...
        with col_hist:
            # apply a pending auto-suggestion before the slider is instantiated
            # (widget keys cannot be written after instantiation in the same run)
            pending_key = f'pending_gate_{marker}'
            if pending_key in st.session_state:
                st.session_state[f'slider_{marker}'] = st.session_state.pop(pending_key)
            thresh = st.slider(
                f'{marker} threshold',
            ...
            with col_auto:
                if st.button('Auto-suggest', key=f'auto_{marker}', help='Run GMM and suggest threshold'):
                    ...
                    if fresh_gmm:
                        suggested = float(fresh_gmm['suggested_threshold'])
                        st.session_state.gates[marker] = suggested
                        # the slider keeps its widget state across reruns; stage the
                        # suggestion and apply it before the slider on the next run
                        st.session_state[f'pending_gate_{marker}'] = suggested
